...
static const char *HELLO =
    "{\"type\":\"hello\",\"protocol_version\":1,\"device_id\":\"mac-test\"," 
    "\"platform\":\"macos\",\"app\":\"StatsForKVM\"}";

static const char *TELEMETRY =
    "{\"type\":\"telemetry\",\"protocol_version\":1,\"device_id\":\"mac-test\"," 
...
static protocol_message_t parse_ok(const char *json)
{
    ...
    const protocol_status_t status = protocol_parse_message(json, strlen(json), &message,
                                                             error, sizeof(error));
...
static void expect_status(const char *json, protocol_status_t expected)
{
    protocol_message_t message;
    char error[PROTOCOL_ERROR_MAX] = {0};
...
static char *replace_once(const char *input, const char *needle, const char *replacement)
{
    ...
    const size_t prefix = (size_t)(position - input);
    const size_t length = prefix + strlen(replacement) +
                          strlen(position + strlen(needle)) + 1;
...
int main(void)
{
    ...
    assert(strcmp(cpu_null_message.data.telemetry.cpu_temperature.error,
                  "value_missing") == 0);
    free(cpu_null);

    char *memory_oob = replace_once(TELEMETRY, "\"value\":50.0,\"unit\":\"percent\"",
                                    "\"value\":101,\"unit\":\"percent\"");
    protocol_message_t oob = parse_ok(memory_oob);
    assert(!oob.data.telemetry.memory_usage.valid);
    assert(strcmp(oob.data.telemetry.memory_usage.error, "value_out_of_range") == 0);
    free(memory_oob);

    char *wrong_version = replace_once(HELLO, "\"protocol_version\":1",
                                       "\"protocol_version\":2");
    expect_status(wrong_version, PROTOCOL_ERROR_UNSUPPORTED_VERSION);
    free(wrong_version);

    const char *confirmed =
        "{\"type\":\"command_result\",\"protocol_version\":1,"
        "\"device_id\":\"mac-test\",\"request_id\":\"esp32-1\","
        "\"success\":true,\"write_succeeded\":true,\"confirmed\":true,"
        "\"requested_input\":7,\"read_back_input\":7,\"error\":null}";
    assert(parse_ok(confirmed).data.command_result.confirmed);
    const char *unconfirmed =
        "{\"type\":\"command_result\",\"protocol_version\":1,"
        "\"device_id\":\"mac-test\",\"request_id\":\"esp32-2\","
        "\"success\":true,\"write_succeeded\":true,\"confirmed\":false,"
        "\"requested_input\":7,\"read_back_input\":null,"
        "\"error\":\"input_written_but_unconfirmed\"}";
    assert(!parse_ok(unconfirmed).data.command_result.confirmed);

    char oversized[PROTOCOL_MAX_MESSAGE_BYTES + 2];
    memset(oversized, 'x', sizeof(oversized));
    protocol_message_t unused;
    assert(protocol_parse_message(oversized, sizeof(oversized), &unused,
                                  error, sizeof(error)) == PROTOCOL_ERROR_TOO_LONG);
    assert(!protocol_is_valid_utf8((const uint8_t *)"\xc0\xaf", 2));
