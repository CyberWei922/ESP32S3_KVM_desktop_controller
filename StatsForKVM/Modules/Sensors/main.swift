...
public class Sensors: Module {
    ...
    /// Keeps the existing Sensors reader active for Monitor without enabling
    /// an extra menu-bar item or creating a second hardware reader.
    public func setKVMTelemetryActive(_ state: Bool) {
    ...
    private func usageCallback(_ raw: Sensors_List?) {
        guard let value = raw else {
            KVMTelemetryStore.shared.updateAverageCPUTemperature(nil, error: "sensor_read_failed")
            return
        }

        let averageCPU = value.sensors.first(where: { $0.key == "Average CPU" })?.value
        KVMTelemetryStore.shared.updateAverageCPUTemperature(averageCPU)
