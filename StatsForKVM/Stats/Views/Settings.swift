...
class SettingsWindow: NSWindow, NSWindowDelegate, NSToolbarDelegate {
    ...
    private static let frameAutosaveName = "com.wei.statsforkvm.Settings.WindowFrame"
    ...
    private var monitorSettings: MonitorSettingsView = MonitorSettingsView(frame: .zero)
    private var kvmSettings: KVMSettingsView = KVMSettingsView(frame: .zero)
    ...
    init() {
        ...
        let newToolbar = NSToolbar(identifier: "com.wei.statsforkvm.Settings.Toolbar")
    ...
    @objc private func menuCallback(_ notification: Notification) {
        if let title = notification.userInfo?["module"] as? String {
            ...
            if let detectedModule = modules.first(where: { $0.config.name == title }) {
                ...
                self.monitorSettings.viewWillAppear()
                view = self.monitorSettings
                self.toggleButton?.isHidden = true
                self.settingsPreviewButton?.isHidden = true
                NotificationCenter.default.post(name: .openWindow, object: nil, userInfo: ["state": false])
            } else if title == "KVM" {
                self.kvmSettings.viewWillAppear()
                view = self.kvmSettings
                self.toggleButton?.isHidden = true
                self.settingsPreviewButton?.isHidden = true
                NotificationCenter.default.post(name: .openWindow, object: nil, userInfo: ["state": false])
            }
...
private class SidebarView: NSStackView {
    ...
    private var monitorIcon: NSImage { iconFromSymbol(name: "waveform.path.ecg", scale: .large) }
    private var kvmIcon: NSImage { iconFromSymbol(name: "display.2", scale: .large) }
    ...
    override init(frame: NSRect) {
        ...
        self.scrollView.stackView.addArrangedSubview(MenuItem(icon: self.monitorIcon, title: "Monitor"))
        self.scrollView.stackView.addArrangedSubview(MenuItem(icon: self.kvmIcon, title: "KVM"))
...
private class MenuItem: NSView {
    ...
    init(icon: NSImage?, title: String) {
        ...
        if title == "Settings" {
            ...
            toolTip = localizedString("Open Monitor settings")
        } else if title == "KVM" {
            toolTip = localizedString("Open KVM settings")
        } else {
