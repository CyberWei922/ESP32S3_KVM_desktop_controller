...
CGDisplayCount getOnlineDisplayInfos(DisplayInfos* displayInfos) {
    ...
    for (int i = 0; i < (int)screenCount && validDisplayCount < MAX_DISPLAYS; i++) {
        ...
        currDisplay->uuid = [(NSString *)CFDictionaryGetValue(displayInfoDict, CFSTR("kCGDisplayUUID")) copy];
        currDisplay->ioLocation = [(NSString *)CFDictionaryGetValue(displayInfoDict, CFSTR("IODisplayLocation")) copy];
        ...
        if (currDisplay->uuid == NULL || currDisplay->ioLocation == NULL) {
            [currDisplay->uuid release];
            [currDisplay->ioLocation release];
        ...
        if (displayAttrs) {
            ...
            if (productAttrs) {
                currDisplay->productName = [([productAttrs objectForKey:@"ProductName"] ?: @"Unknown Display") copy];
                currDisplay->manufacturer = [[productAttrs objectForKey:@"ManufacturerID"] copy];
                currDisplay->alphNumSerial = [[productAttrs objectForKey:@"AlphanumericSerialNumber"] copy];
            }
...
void releaseDisplayInfos(DisplayInfos* displayInfos, CGDisplayCount count) {
    for (CGDisplayCount index = 0; index < count; index++) {
        if (displayInfos[index].adapter != MACH_PORT_NULL) {
            IOObjectRelease(displayInfos[index].adapter);
            displayInfos[index].adapter = MACH_PORT_NULL;
        }
        [displayInfos[index].uuid release];
        [displayInfos[index].ioLocation release];
        [displayInfos[index].edid release];
        [displayInfos[index].productName release];
        [displayInfos[index].manufacturer release];
        [displayInfos[index].alphNumSerial release];
        displayInfos[index].uuid = nil;
        displayInfos[index].ioLocation = nil;
        displayInfos[index].edid = nil;
        displayInfos[index].productName = nil;
        displayInfos[index].manufacturer = nil;
        displayInfos[index].alphNumSerial = nil;
    }
}
