...
static void reconnect_timer_callback(void *argument)
{
    ...
    if (result != ESP_OK) ESP_LOGW(TAG, "Reconnect start failed: %s", esp_err_to_name(result));
}

static void schedule_reconnect(void)
{
    if (s_reconnect_timer == NULL || esp_timer_is_active(s_reconnect_timer)) return;
    ESP_LOGI(TAG, "Wi-Fi reconnect scheduled in %" PRIu32 " ms", s_reconnect_delay_ms);
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        esp_err_t result = esp_wifi_connect();
        if (result != ESP_OK) ESP_LOGW(TAG, "Initial connect failed: %s", esp_err_to_name(result));
        return;
    }
    ...
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        app_state_set_wifi(s_app_state, true, address, "WIFI CONNECTED");
        ESP_LOGI(TAG, "Wi-Fi connected, DHCP address: %s", address);
        esp_err_t result = websocket_server_start();
        if (result != ESP_OK) {
            ESP_LOGE(TAG, "WebSocket server failed: %s", esp_err_to_name(result));
            app_state_set_last_message(s_app_state, "WEBSOCKET START FAILED");
        }
    }
}
...
esp_err_t network_manager_init(app_state_t *state)
{
    ...
    if (!configured) {
        ...
        ESP_LOGW(TAG, "wifi_not_configured: create main/include/kvm_config.local.h");
        return ESP_OK;
    }
    ...
    ESP_RETURN_ON_ERROR(esp_timer_create(&timer_args, &s_reconnect_timer), TAG,
                        "reconnect timer init failed");

    wifi_config_t wifi_config = {0};
    strlcpy((char *)wifi_config.sta.ssid, KVM_WIFI_SSID, sizeof(wifi_config.sta.ssid));
    strlcpy((char *)wifi_config.sta.password, KVM_WIFI_PASSWORD,
            sizeof(wifi_config.sta.password));
    wifi_config.sta.threshold.authmode = KVM_WIFI_PASSWORD[0] == '\0'
    ...
    ESP_LOGI(TAG, "Wi-Fi station started (credentials are intentionally not logged)");
    return ESP_OK;
}
