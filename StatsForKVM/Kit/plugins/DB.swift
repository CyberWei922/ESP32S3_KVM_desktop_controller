...
public class DB {
    ...
    private let queue = DispatchQueue(label: "com.wei.statsforkvm.db")
    ...
    init() {
        ...
        let supportPath = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("StatsForKVM")
        let tmpPath = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("StatsForKVM")
