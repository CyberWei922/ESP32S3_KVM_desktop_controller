...
这是 Windows 端常驻程序的独立交接目录。目标是让另一台 Windows 电脑上的开发 Agent 只依赖本目录，就能实现与现有 ESP32 固件和 macOS `StatsForKVM` 客户端配套的 Windows 客户端。

本目录保留初版需求、协议、实现边界、配置和验收标准；V3 源码位于上级 `src`，自动测试位于上级 `tests`，发布包位于上级 `publish/v3-win-x64-single-file`。V3 交付状态以 `V3_UPGRADE_REPORT.md` 为准。
...
- 当前 ESP32 仍处于仅允许 Mac → Windows 的临时测试模式。Windows 真机验收完成后才能关闭该模式。

## 当前最重要的兼容性前置条件

现有 ESP32 `0.5.0-dev.2` 已识别 `platform: "windows"`，但其遥测解析器仍把 Mac 的来源名称硬编码为 `Average CPU` 和 `Stats RAM_Usage`。Windows 正式客户端必须如实发送：

- CPU：`LibreHardwareMonitor CPU Package`
- 内存：`GlobalMemoryStatusEx`

因此，在 Windows 客户端接入 ESP32 之前，必须把 ESP32 改为按 `platform` 校验来源名称。不得让 Windows 客户端伪装成 macOS，也不得发送虚假的 Mac 来源名称。具体见 [PROJECT_SPEC.md](PROJECT_SPEC.md) 的“ESP32 接入前置修改”。

## 权威性

本目录记录的是 Windows v1 的目标合同。若旧文件与本目录冲突：
