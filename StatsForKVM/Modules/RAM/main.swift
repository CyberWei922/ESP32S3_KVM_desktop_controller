...
public class RAM: Module {
    ...
    /// Keeps the existing RAM reader active for Monitor without enabling an
    /// extra menu-bar item or creating a duplicate reader.
    ...
    private func loadCallback(_ raw: RAM_Usage?) {
        guard let value = raw else {
            KVMTelemetryStore.shared.updateMemoryUsage(fraction: nil, error: "memory_read_failed")
            return
        }

        KVMTelemetryStore.shared.updateMemoryUsage(fraction: value.usage)
