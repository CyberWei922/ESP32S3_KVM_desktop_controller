...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    internal let updateActivity = NSBackgroundActivityScheduler(identifier: "com.wei.statsforkvm.updateCheck")
    internal let supportActivity = NSBackgroundActivityScheduler(identifier: "com.wei.statsforkvm.support")
    internal let supportRetryActivity = NSBackgroundActivityScheduler(identifier: "com.wei.statsforkvm.supportRetry")
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        MonitorService.shared.onTelemetryRequirementChange = { active in
            (modules.first(where: { $0 is Sensors }) as? Sensors)?.setKVMTelemetryActive(active)
            (modules.first(where: { $0 is RAM }) as? RAM)?.setKVMTelemetryActive(active)
        }
        DisplayController.shared.startCommandListener()
        ...
        self.setup {
            ...
            MonitorService.shared.startIfEnabled()
        ...
        NSWorkspace.shared.notificationCenter.addObserver(
            self,
            selector: #selector(handleWorkspaceWake),
    ...
    func applicationWillTerminate(_ aNotification: Notification) {
        MonitorService.shared.shutdown()
    ...
    @objc private func handleWorkspaceWake() {
        MonitorService.shared.applicationDidWake()
    }
